...
def test_workout_serializer_update(create_workout):
    ...
    serializer = WorkoutSerializer(instance=create_workout, data={"workout_name": "New Workout"}, partial=True)
...
def test_workout_serializer_read_only_fields(create_workout):
    ...
    serializer = WorkoutSerializer(instance=create_workout, data={"user": None}, partial=True)
...
def test_setdict_serializer_read_only_fields(create_setdict):
    """Test that read-only fields cannot be updated"""
    serializer = SetDictSerializer(instance=create_setdict, data={"set_number": 99, "set_order": 99}, partial=True)

    assert serializer.is_valid(), serializer.errors
    updated_set = serializer.save()

    assert updated_set.set_number == create_setdict.set_number  # Should remain unchanged
