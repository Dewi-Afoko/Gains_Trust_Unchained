...
def test_register_valid_data(api_client, user_data):
    ...
    assert (
        f"{user_data['username']} successfully registered!" in response.data["message"]
    )
...
def test_logout_invalid_token(api_client, create_user):
    ...
    response = api_client.post(
        reverse("logout"), {"refresh": "invalid_token"}, format="json"
    )
...
def test_update_user_valid_data(api_client, create_user):
    ...
    assert "message" in response.data
    assert response.data["message"] == f"Details for {create_user.username}"
...
def test_delete_weight_authenticated(api_client, create_user, create_weight):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert "message" in response.data
    assert f"Weight ID: {create_weight.id} has been deleted" in response.data["message"]
