...
const TimerLive = ({
    ...
    const {
        ...
        stopRestTimer,
        hydrateRestTimer,
        markSetAsManuallyStarted,
        isSetManuallyStarted,
        clearManualSetStart,
    } = useTimerStore()
    ...
    useEffect(() => {
        if (nextSet?.id && workoutStarted && !isResting) {
            const savedSetStartTime = localStorage.getItem(
                `setStartTime_${nextSet.id}`
            )
            if (savedSetStartTime) {
                const elapsed = Math.floor(
                    (Date.now() - parseInt(savedSetStartTime)) / 1000
                )
    ...
    const handleCompleteSet = () => {
        if (nextSet?.id) {
            ...
            localStorage.setItem(
                `completedSetDuration_${nextSet.id}`,
                duration.toString()
            )
    ...
    return (
        <>
            ...
            {isResting &&
                ...
                createPortal(
                    <div
                        className="fixed inset-0 z-[999999] bg-gradient-to-br from-red-600/90 to-orange-600/90 backdrop-blur-sm flex items-center justify-center"
            ...
            >
                ...
                <div className="relative z-20 h-full flex flex-col">
                    ...
                    >
                        ...
                        {isResting && (
                            <div
                                className={`mb-12 p-12 rounded-2xl shadow-2xl relative overflow-hidden transition-all duration-300 ${
                                    restIntensity === 'critical'
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                    ...
                                    <p
                                        className={`font-mono font-black mb-8 transition-all duration-300 ${
                                            restIntensity === 'critical'
                                    ...
                                    <p
                                        className={`font-extrabold uppercase tracking-wider transition-all duration-300 ${
                                            restIntensity === 'critical'
