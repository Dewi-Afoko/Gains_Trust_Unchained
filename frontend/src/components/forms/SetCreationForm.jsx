...
const SetCreationForm = ({ onClose }) => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const numberOfSets = addMultipleSets ? parseInt(data.set_count, 10) : 1; // ✅ Default to 1 if unchecked
    ...
    return (
        <div className="bg-[#600000] text-white p-6 rounded-lg shadow-lg max-w-md fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[1000]">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <label className="block">Exercise Name:<input {...register('exercise_name', { required: true })} className="w-full p-2 mt-1 rounded text-black" /></label>
                ...
                <label className="flex items-center space-x-2">
                    <input type="checkbox" {...register('addMultipleSets')} className="text-yellow-400" />
                ...
                {addMultipleSets && (
                    <label className="block">
                        Number of Sets:
                        <input type="number" {...register('set_count')} min="1" defaultValue="1" className="w-full p-2 mt-1 rounded text-black" />
                    </label>
                )}

                <button type="submit" className="w-full bg-yellow-400 text-black font-bold p-2 rounded hover:bg-yellow-300">{isSubmitting ? 'Saving...' : 'Add Set(s)'}</button>
                <button type="button" className="w-full bg-gray-500 text-white font-bold p-2 rounded hover:bg-gray-400 mt-2" onClick={onClose}>Cancel</button>
            </form>
        </div>
    )
}
